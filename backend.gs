...
function getProductStructures(ss) {
  ...
  // Row 1 is Header: LSX | Product | Structure
  ...
  if (data.length > 1) {
    for(let i = 1; i < data.length; i++) {
      const lsx = data[i][0] ? String(data[i][0]).trim() : "";
      const product = data[i][1] ? String(data[i][1]).trim() : "";
      const structure = data[i][2] ? String(data[i][2]).trim() : "";
      
      if(product) products.add(product);
      if(structure) structures.add(structure);
      
      if(lsx || product || structure) {
        mappings.push({ lsx, product, structure });
      }
    }
  }
...
function saveProductStructureIfNew(ss, lsx, product, structure) {
  const sheet = getOrCreateSheet(ss, "Product_Structure");
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(["LSX", "Product", "Structure"]);
  ...
  for(let i = 1; i < data.length; i++) {
    if (String(data[i][1]) === String(product) && String(data[i][2]) === String(structure)) {
      exists = true;
      break;
    }
  }
  
  if ((product || structure) && !exists) {
    sheet.appendRow([lsx || "", product || "", structure || ""]);
  }
}
...
function saveStandard(ss, payload) {
  ...
  const payloadId = payload.id ? String(payload.id).trim() : "";

  // 1. Tìm bằng ID trước (ưu tiên khi Edit)
  if (payloadId && idIdx !== -1) {
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][idIdx]).trim() === payloadId) {
        rowIndex = i + 1;
        oldProductName = nameIdx !== -1 ? data[i][nameIdx] : "";
        oldStructure = structIdx !== -1 ? data[i][structIdx] : "";
        break;
      }
    }
  }
  ...
  if (rowIndex === -1 && nameIdx !== -1 && structIdx !== -1) {
    for (let i = 1; i < data.length; i++) {
      if (data[i][nameIdx] === payload.productName && data[i][structIdx] === payload.structure) {
        rowIndex = i + 1;
        oldProductName = data[i][nameIdx];
        oldStructure = data[i][structIdx];
        break;
      }
    }
  }
  ...
  // If updating, preserve existing data
  ...
  if (idIdx !== -1) newRow[idIdx] = payload.id || payloadId;
  if (nameIdx !== -1) newRow[nameIdx] = payload.productName;
  if (structIdx !== -1) newRow[structIdx] = payload.structure;
...
function saveLog(ss, payload) {
  ...
  // Updated System Keys
  const systemKeys = ["action", "timestamp", "product", "structure", "productStd", "structureStd", "productionOrder", "machineId", "machineName", "uploadedBy", "model", "sendEmail", "emailTo", "emailHtml", "emailSubject"];
  ...
  saveProductStructureIfNew(ss, payload.productionOrder, payload.product, payload.structure);
  ...
  if (payload.sendEmail && payload.emailTo && payload.emailHtml) {
    try {
      MailApp.sendEmail({
        to: payload.emailTo,
        subject: payload.emailSubject || "Cảnh báo thông số máy",
        htmlBody: payload.emailHtml
      });
    } catch (e) {
